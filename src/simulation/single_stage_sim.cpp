...
#pragma omp parallel for
    ...
    rhs(0) = excitation_->voltage(excitation) - R_d_ * coil_current -
             velocity * motional_emf;
    for (int filament = 0; filament < N_fil_; ++filament) {
    ...
    result.force = compute_force(state, gradient);
...
void SingleStageSim<SP>::record_step(double post_time) {
    const auto derivative = evaluate_derivatives(
        integration_state_.physical, *integration_state_.excitations.front(), workspace_);
    SimStep entry;
    ...
    entry.force = integration_state_.stages.front().circuit_active ? derivative.force : 0.0;
